...
def dashboard():
    ...
    # 1. Fetch KPI metrics
    # Revenue leakage (unmatched active shifts)
    cursor.execute("""
        SELECT COALESCE(SUM(discrepancy_amount), 0.0) as val 
        FROM reconciliation_results 
        WHERE discrepancy_type = 'unbilled_shift'
    """)
    leakage = cursor.fetchone()['val']
    
    # Invoice Error cost (unmatched invoices + amount/duration discrepancies)
    cursor.execute("""
        SELECT COALESCE(SUM(discrepancy_amount), 0.0) as val 
        FROM reconciliation_results 
        WHERE discrepancy_type IN ('missing_shift', 'duration_mismatch', 'rate_mismatch')
    """)
    ...
    # Discrepancy breakdown by participant
    cursor.execute("""
        SELECT 
        ...
        WHERE r.match_status != 'matched' OR r.discrepancy_type != 'none'
    ...
    # Discrepancy breakdown by staff member (ShiftCare)
    cursor.execute("""
        SELECT 
            s.staff_id,
            COUNT(r.result_id) as flag_count,
...
def unmatched_shifts():
    ...
    # Fetch unmatched or partially matched shifts
    cursor.execute("""
        SELECT 
            r.result_id,
            r.match_status,
        ...
        FROM reconciliation_results r
        JOIN shifts s ON r.shift_id = s.shift_id
        WHERE r.match_status = 'unmatched' OR (r.match_status = 'partial' AND r.discrepancy_type = 'duration_mismatch' AND r.discrepancy_amount > 0)
        ORDER BY r.discrepancy_amount DESC
    """)
    results = cursor.fetchall()
    conn.close()
    
    return render_template('unmatched_shifts.html', user=get_current_user(), results=results)
...
def invoice_errors():
    ...
    # Fetch invoice errors (missing shifts or rate/amount discrepancies)
    cursor.execute("""
        SELECT 
            r.result_id,
            r.match_status,
        ...
        LEFT JOIN shifts s ON r.shift_id = s.shift_id
        WHERE r.match_status = 'unmatched' OR r.discrepancy_type IN ('rate_mismatch', 'duration_mismatch')
        ORDER BY r.discrepancy_amount DESC
    """)
    results = cursor.fetchall()
    conn.close()
    
    return render_template('invoice_errors.html', user=get_current_user(), results=results)
