...
def process_match(cursor, shift, inv, base_status, tier_name, price_rules):
    ...
    if abs(duration_diff) > 0.05: # ignore rounding errors
        base_status = "partial"
        if duration_diff > 0:
            # Underbilled: Shift duration was longer than billed quantity
            discrepancy_type = "duration_mismatch"
            discrepancy_amount = duration_diff * inv["unit_amount"]
            severity = "medium"
            notes += f" Underbilled by {duration_diff:.2f} hours (leakage)."
        else:
            # Overbilled: Billed quantity was longer than shift duration
            discrepancy_type = "duration_mismatch"
            discrepancy_amount = abs(duration_diff) * inv["unit_amount"]
            severity = "medium"
            notes += f" Overbilled client by {abs(duration_diff):.2f} hours."
    ...
    if rule:
        ...
        if inv["unit_amount"] > limit + 0.01:
            ...
            discrepancy_amount = max(discrepancy_amount, rate_diff * inv["quantity"])
            severity = "high"
            notes += f" Invoiced rate ${inv['unit_amount']:.2f} exceeds NDIS guide max ${limit:.2f}."
